...
#clf_up = svm.SVC(decision_function_shape='ovo')
...
#clf_pred_up = clf_up.predict(X_test)

### Get performance metrics
clf_score = metrics.accuracy_score(y_test, clf_pred) * 100
#clf_score_up = metrics.accuracy_score(y_test, clf_pred_up) * 100
...
print("Classification report for :\n{}".format(metrics.classification_report(y_test, clf_pred_up)))
print("Accuracy score:", clf_score_up)
'''
### Plot confusion matrix
confmatrix(clf_pred, "Confusion Matrix\nSVM - Regular Training Set")
...
print("Cross-Validation score for SVM:", clf_cv)
classifier = OneVsRestClassifier(svm.SVC(C=1.0, kernel='rbf', degree=3, gamma='scale', coef0=0.0, shrinking=True, probability=False, tol=0.001, cache_size=200, class_weight=None, verbose=False, max_iter=-1, decision_function_shape='ovr', break_ties=False, random_state=None))
#y_score = classifier.fit(X_train, y_train).decision_function(X_test)
#y_score = classifier.fit(X_train, y_train).decision_function(X_test)
cvscore(clf,X,y)
